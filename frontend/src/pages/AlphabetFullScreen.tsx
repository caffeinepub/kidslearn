...
const BG_COLORS = [
  ...
  '#8BC34A', '#FF9800', '#E040FB', '#26C6DA', '#EF5350',
  '#66BB6A', '#AB47BC', '#42A5F5', '#FFCA28', '#26A69A',
  '#EC407A', '#7E57C2', '#29B6F6', '#FF7043', '#9CCC65',
  '#5C6BC0',
];
...
export default function AlphabetFullScreen() {
  ...
  return (
    ...
    >
      ...
      >
        <ArrowLeft size={22} />
        <span className="text-base font-heading">Back</span>
      </button>

      {/* Main clickable area */}
      <button
        onClick={handleSpeak}
        className="flex flex-col items-center justify-center gap-6 cursor-pointer group focus:outline-none"
      ...
      >
        ...
        <div className="flex items-center justify-center">
          {current.image ? (
            <img
              src={current.image}
              alt={current.word}
              className="w-44 h-44 md:w-60 md:h-60 object-contain drop-shadow-xl rounded-3xl bg-white/20 p-2"
        ...
        {/* Speaker affordance */}
        <div
          className={`flex items-center gap-2 bg-white/25 hover:bg-white/40 text-white px-6 py-3 rounded-full transition-all shadow-md mt-1 ${isSpeaking ? 'animate-pulse scale-110' : 'group-hover:scale-105'}`}
        >
          <Volume2 size={26} />
          <span className="font-body text-lg font-bold">Tap to hear!</span>
        </div>
      </button>
      ...
      <button
        onClick={goPrev}
        className="absolute left-3 md:left-6 top-1/2 -translate-y-1/2 z-20 bg-white/30 hover:bg-white/50 backdrop-blur-sm text-white rounded-full p-4 md:p-5 shadow-xl transition-all hover:scale-110 active:scale-95"
      ...
      <button
        onClick={goNext}
        className="absolute right-3 md:right-6 top-1/2 -translate-y-1/2 z-20 bg-white/30 hover:bg-white/50 backdrop-blur-sm text-white rounded-full p-4 md:p-5 shadow-xl transition-all hover:scale-110 active:scale-95"
