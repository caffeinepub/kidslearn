...
export default function TimedChallenge() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: timer effect intentionally omits deps to avoid recreating the interval on every state change
  useEffect(() => {
      ...
      <div className="min-h-screen bg-gradient-to-br from-sunshine-100 via-tangerine-100 to-cherry-100 flex items-center justify-center p-4">
        ...
        <div className="kid-card border-4 bg-white border-sunshine-400 max-w-md w-full p-8 text-center">
          ...
          <p className="font-body text-2xl text-gray-600 mb-4">
            Score:{" "}
            <span className="font-heading text-cherry-500 text-3xl">
              {score}
            </span>{" "}
            / {questions.length}
          </p>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-sunshine-100 via-tangerine-100 to-cherry-100 flex items-center justify-center p-4">
        <div className="kid-card border-4 bg-white border-tangerine-400 max-w-md w-full p-8 text-center">
          ...
          <h1 className="font-heading text-5xl text-tangerine-600 mb-4">
            Timed Challenge!
          </h1>
          <p className="font-body text-xl text-gray-600 mb-6">
            Answer {questions.length} questions as fast as you can! You have{" "}
    ...
    <div className="min-h-screen bg-gradient-to-br from-sunshine-100 via-tangerine-100 to-cherry-100 py-6 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-6">
          <div
            className={`kid-card border-4 px-5 py-3 flex items-center gap-2 ${timeLeft <= 5 ? "bg-cherry-200 border-cherry-500" : "bg-sunshine-200 border-sunshine-500"}`}
          >
            <Timer
              size={28}
              className={
                timeLeft <= 5
                  ? "text-cherry-600 animate-pulse"
                  : "text-sunshine-600"
              }
            />
            <span
              className={`font-heading text-3xl ${timeLeft <= 5 ? "text-cherry-700" : "text-sunshine-700"}`}
            >
              {timeLeft}s
            </span>
          </div>
          ...
          <div className="kid-card border-4 bg-grass-200 border-grass-500 px-5 py-3">
            <span className="font-heading text-3xl text-grass-700">
              ⭐ {score}
            </span>
          </div>
        </div>
        ...
        <div className="text-center mb-4">
          <span className="font-heading text-2xl text-gray-600">
        ...
        <div className="kid-card border-4 bg-white border-sky-300 p-8 mb-6">
          <p className="font-heading text-3xl md:text-4xl text-gray-800 text-center">
            {question.question}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {question.options.map((option, idx) => (
            <button
              key={`option-${idx}-${option.slice(0, 8)}`}
              type="button"
