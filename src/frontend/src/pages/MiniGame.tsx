...
import { useAwardBadge, useGetMiniGameContent } from "../hooks/useQueries";
...
function EarlyLearnerGame({ onWin }: { onWin: () => void }) {
  ...
  const handleDrop = (e: React.DragEvent, targetLetter: string) => {
    ...
    if (Object.keys(newMatches).length === LETTER_PAIRS.length) {
      const allCorrect = LETTER_PAIRS.every(
        (p) => newMatches[p.letter] === p.letter,
      );
  ...
  const isWrong = (letter: string) =>
    !!(matches[letter] && matches[letter] !== letter);
  ...
  return (
    <div>
        ...
        {shuffledWords.map((pair) => (
          ...
          >
            <div
              ...
              ${
                isCorrect(pair.letter)
                  ? "bg-grass-300 border-grass-500 text-white"
                  : isWrong(pair.letter)
                    ? "bg-cherry-300 border-cherry-500 text-white"
      ...
      {Object.keys(matches).length > 0 &&
        !LETTER_PAIRS.every((p) => matches[p.letter] === p.letter) && (
          <button
            type="button"
            onClick={() => setMatches({})}
...
function OlderKidsGame({ onWin }: { onWin: () => void }) {
  ...
  const [letters, setLetters] = useState<
    Array<{ char: string; id: number; used: boolean }>
  >([]);
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: currentWord.word is derived from wordIndex, listing both causes double-trigger
  useEffect(() => {
  ...
  const handleAnswerClick = (letterId: number) => {
    ...
    setLetters((prev) =>
      prev.map((l) => (l.id === letterId ? { ...l, used: false } : l)),
    );
    setIsCorrect(null);
  };
  ...
  return (
    <div>
      ...
      >
        ...
        {answer.length === 0 && (
          <span className="font-nunito text-muted-foreground text-sm self-center">
      ...
      {isCorrect === true && (
        <p className="text-center font-fredoka text-xl text-grass-600 mb-3">
          🎉 Correct! Well done!
        </p>
      )}
      {isCorrect === false && (
        <p className="text-center font-fredoka text-xl text-cherry-600 mb-3">
          ❌ Not quite! Try again!
        </p>
      )}
      ...
      {hasWon ? (
        <div className="animate-bounce-in text-center">
          <div className="bg-sunshine-400 border-4 border-sunshine-600 rounded-3xl p-8 mb-6">
            ...
            <h2 className="font-fredoka text-3xl text-foreground mb-2">
              You Won!
            </h2>
            <p className="font-nunito font-bold text-foreground/80">
              Amazing work! You completed the game!
            </p>
          </div>
