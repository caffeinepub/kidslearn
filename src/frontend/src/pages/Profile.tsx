...
export default function Profile() {
  ...
  const { data: displayName, isLoading: nameLoading } =
    useGetDisplayName(principalText);
  const { mutateAsync: setDisplayName, isPending: isSaving } =
    useSetDisplayName();
  const { data: progress, isLoading: progressLoading } =
    useGetSessionProgress(principalText);
  ...
  const roleLabel = role === UserRole.parent ? "👨‍👩‍👧 Parent" : "🎓 Student";
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-lavender-100 via-sky-100 to-sunshine-100 flex items-center justify-center px-4">
        <div className="bg-white border-4 border-lavender-300 rounded-4xl p-10 shadow-fun-xl text-center max-w-md w-full">
          ...
          <h1 className="font-fredoka text-3xl text-lavender-700 mb-2">
            My Profile
          </h1>
          <p className="font-nunito text-muted-foreground mb-6">
            Log in to view your profile, track your learning progress, and earn
            badges!
          </p>
          <button
            type="button"
    ...
    <div className="min-h-screen bg-gradient-to-br from-lavender-100 via-sky-100 to-sunshine-100 px-4 py-8">
      <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="inline-block bg-lavender-100 border-2 border-lavender-300 rounded-full px-3 py-0.5 mb-2">
                <span className="font-nunito text-sm font-bold text-lavender-700">
                  {roleLabel}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                >
                  {copiedPrincipal ? (
                    <CheckCircle size={14} className="text-grass-500" />
                  ) : (
                    <Copy size={14} />
                  )}
                </button>
          ...
          {progressLoading ? (
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-3">
              {(
                ...
                <div
                  key={k}
                  className="h-16 bg-sunshine-50 rounded-2xl animate-pulse"
                />
              ))}
            </div>
          ) : (
            ...
            {progressLoading ? (
              <div className="space-y-3">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-12 bg-grass-50 rounded-2xl animate-pulse"
                  />
                ))}
              </div>
            ) : (
              <div className="space-y-3">
                {quizResults.map((result, idx) => {
                  ...
                  return (
                    <div
                      key={`quiz-result-${idx}-${result.subject}`}
            ...
            {progressLoading ? (
              <div className="flex flex-wrap gap-2">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-8 w-20 bg-sky-50 rounded-full animate-pulse"
                  />
                ))}
              </div>
            ) : (
        ...
        <div className="bg-gradient-to-r from-cherry-400 to-tangerine-400 border-4 border-cherry-500 rounded-4xl p-5 shadow-fun-xl text-center">
          <p className="font-fredoka text-xl text-white mb-2">
            ❤️ Enjoying KidsLearn?
          </p>
