...
export default function PuzzleGame() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: currentWord derives from wordIdx/language, no need to include it separately
  useEffect(() => {
    if (currentWord) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-coral-100 via-sunshine-100 to-tangerine-100 py-6 px-4">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-6">
          <h1 className="font-heading text-5xl text-coral-600 drop-shadow-md mb-2">
            🧩 Puzzle Game!
          </h1>
          <p className="font-body text-xl text-coral-500 font-semibold">
        ...
        <div className="kid-card border-4 bg-white border-sky-300 p-6 mb-6 text-center">
          <p className="font-heading text-2xl text-gray-700 mb-2">
            Spell this word:
          </p>
          <span className="text-6xl">{currentWord?.emoji}</span>
          <p className="font-heading text-xl text-sky-600 mt-2">
            {currentWord?.hint}
          </p>
        </div>
        ...
        <div className="kid-card border-4 bg-grass-100 border-grass-400 p-4 mb-4 min-h-[80px] flex items-center justify-center gap-2 flex-wrap">
          {selected.length === 0 ? (
            <span className="font-body text-gray-400 text-lg">
              Tap letters below...
            </span>
          ) : (
            selected.map((letter, idx) => (
              <span
                // biome-ignore lint/suspicious/noArrayIndexKey: position is the identity for selected letter slots
                key={`selected-${idx}`}
        ...
        {!solved && (
          <div className="flex flex-wrap justify-center gap-3 mb-6">
            {tiles.map((letter, idx) => (
              <button
                // biome-ignore lint/suspicious/noArrayIndexKey: shuffled tile positions are index-stable
                key={`tile-${idx}`}
                type="button"
