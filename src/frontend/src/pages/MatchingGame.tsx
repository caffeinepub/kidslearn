...
export default function MatchingGame() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: initGame is stable and doesn't need to be in deps
  useEffect(() => {
    initGame(language);
  }, [language]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-mint-100 via-sky-100 to-lavender-100 py-6 px-4">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-6">
          <h1 className="font-heading text-5xl text-mint-600 drop-shadow-md mb-2">
            🃏 Matching Game!
          </h1>
          <p className="font-body text-lg text-mint-500 font-semibold">
            Match the emoji with the word!
          </p>
        </div>
        ...
        <div className="flex justify-center gap-6 mb-6">
          <div className="kid-card border-4 bg-sunshine-200 border-sunshine-500 px-6 py-3 text-center">
            <span className="font-heading text-2xl text-sunshine-700">
              Moves: {moves}
            </span>
          </div>
          <div className="kid-card border-4 bg-grass-200 border-grass-500 px-6 py-3 text-center">
            <span className="font-heading text-2xl text-grass-700">
          ...
          {cards.map((card) => {
            ...
            return (
              <button
                ...
                className={`kid-card border-4 cursor-pointer min-h-[80px] flex items-center justify-center transition-all duration-300 w-full ${
                  matchedState
              ...
              >
                {flippedState ? (
                  <span
                    className={`font-heading text-center px-1 ${card.type === "emoji" ? "text-3xl" : "text-base"}`}
                  >
                    {card.content}
                  </span>
                ) : (
