...
export default function Quiz() {
  ...
  const handleNext = async () => {
    ...
    const finalScore = isLastQuestion
      ? score + (currentCorrect ? 1 : 0)
      : score;
      ...
      if (identity) {
        ...
        try {
          await awardBadge.mutateAsync({
            ...
            badgeId: `quiz-${language}`,
          });
        } catch {
          /* ignore duplicate */
        }
        try {
          await recordQuizResult.mutateAsync({
            subject: language,
        ...
        <div className="kid-card border-4 bg-white border-sunshine-400 max-w-md w-full p-8 text-center">
          ...
          {pct >= 70 && (
            <div className="bg-sunshine-100 border-4 border-sunshine-400 rounded-2xl p-4 mb-4">
              <p className="font-heading text-2xl text-sunshine-700">
                🎖️ Badge Earned!
              </p>
            </div>
          )}
    ...
    <div className="min-h-screen bg-gradient-to-br from-cherry-100 via-sunshine-100 to-sky-100 py-6 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-6">
          <h1 className="font-heading text-5xl md:text-6xl text-cherry-600 drop-shadow-md mb-2">
            ❓ Quiz Time!
          </h1>
        </div>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="font-heading text-xl text-grass-600">
            ⭐ {score}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          {currentQuestion.options.map((option, idx) => {
            ...
            if (showResult && isCorrect)
              extraClass = "bg-grass-300 border-grass-600 scale-105";
            else if (showResult && isSelected && !isCorrect)
              extraClass = "bg-cherry-300 border-cherry-600 opacity-80";
            ...
            return (
              ...
              >
                {showResult && isCorrect && (
                  <CheckCircle className="text-grass-600 shrink-0" size={28} />
                )}
                {showResult && isSelected && !isCorrect && (
                  <XCircle className="text-cherry-600 shrink-0" size={28} />
                )}
                <span>{option}</span>
              </button>
            );
