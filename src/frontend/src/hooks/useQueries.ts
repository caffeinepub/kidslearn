...
import type { GameSession, GameStatistics, Task } from "../backend";
...
export function useRecordQuizResult() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      score,
      total,
    }: { subject: string; score: bigint; total: bigint }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useAwardBadge() {
  ...
  return useMutation({
    mutationFn: async ({
      targetPrincipal,
      badgeId,
    }: { targetPrincipal: string; badgeId: string }) => {
      if (!actor) throw new Error("Actor not available");
      const { Principal } = await import("@dfinity/principal");
...
export function getGuestGameStatistics(gameType: string): {
  ...
  const sessions = getGuestGameSessions().filter(
